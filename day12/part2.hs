...
findStarts :: B -> [Coord]
findStarts = map fst . filter (\(i,e) -> e <= 1) . assocs
...
dfs b ((i,d):is) visited
  ...
  | otherwise = dfs b nis (S.insert i visited)
      where
        ...
        nis = is ++ (map (,d+1) . filter (`S.notMember` visited) $ newDir)

findShortest :: B -> Int
findShortest b = dfs b (map (,0) starts) S.empty
  where
    starts = findStarts b
...
tests = do
  ...
  t "process" $ process testinput `eq` "29"
