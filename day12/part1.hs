...
dfs :: B -> [Coord] -> S.Set Coord -> Int
dfs b [] visited = S.size visited
dfs b (i:is) visited
  | b ! i == 27 = S.size visited
  | i `S.member` visited = dfs b is visited
  | otherwise = dfs b nis (S.insert i visited)
      where
        newDir = findSteps b i
        nis = is ++ filter (`S.notMember` visited) newDir

findShortest :: B -> Int
findShortest b = dfs b [(findStart b)] S.empty
